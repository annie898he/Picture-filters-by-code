...
'''
    ...
    def PPM_greyscale(self):
        ...
        for rowlist in newpixellist:
            col=0
            for pixel in rowlist: #will add red, green, and blue together then divide by 3 to gather the average number
                ...
                col+=1
            row+=1
        print(self.outasciifile+ " output file turned greyscale.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.


    def PPM_flip_horizontal(self):
        ''''flips image horizontally'''
        newpixellist=copy.deepcopy(self.pixellist) #makes a copy where objects are different
        for rowlist in newpixellist:
            rowlist = rowlist.reverse() #will reverse each rowlist in new copy
    ...
    def PPM_rotateclockwise(self):
        ...
        newpixellist=self.pixellist
        rows=len(newpixellist) #finds the length of the rows
        col=len(newpixellist[0]) #finds the length of the columns
        temp=[] #creates a list called temp
        whole=[] #creates a called whole
        for i in range (col):   
            for each in newpixellist:
                if rows == 0: #this is so the first row does not keep repeating
                    rows = len(newpixellist)
                    whole.append(temp)
                    temp=[] #empties out temp list
                num = newpixellist[rows-1][i] #goes to the next row in newpixellist
                temp.append(num) #places num in temp list
                rows = rows-1 #this is so it continues on to the next row
        whole.append(temp) #takes everything in temp list and combines it in whole list
        print(self.outasciifile+ " output file rotated clockwise.")
        self.PPM_updatefrompixellist(whole) # This call will update all member attributes appropriately.

    def PPM_make_purple(self):
        '''makes purple'''
        newpixellist=self.pixellist
        self.width=len(newpixellist[0])
        self.height=len(newpixellist)
        ...
        for rowlist in newpixellist:
            col=0
            for pixel in rowlist: #will add red, green, and blue together then divide by 3 to gather the average number
                newpixellist[row][col][0]=((newpixellist[row][col][0] + newpixellist[row][col][2])/2) # update red
                newpixellist[row][col][1]= 0 # update green
                newpixellist[row][col][2]=((newpixellist[row][col][0] + newpixellist[row][col][2])/2) #update blue
                col+=1
            row+=1
        print(self.outasciifile+ " output file make purple.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.

    def PPM_flip_vertical(self):
        newpixellist=copy.deepcopy(self.pixellist) #makes a copy where objects are different
        newpixellist.reverse() #will reverse newpixellist in new copy
        print(self.outasciifile+ " output file flipped vertically.")
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member attributes appropriately.
